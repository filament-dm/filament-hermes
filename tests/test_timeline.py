...
def test_line_sender_classification_tags():
    ...
    assert "(" not in timeline.render_message_line(_msg(event_id="")).split(":", 1)[
        0
    ].replace("(unknown", "")


def test_line_folds_newlines_and_strips_control_chars():
    line = timeline.render_message_line(_msg(body="line one\nline two\x07\x00"))
    assert "\n" not in line
...
def test_line_neutralizes_unicode_line_separators():
    ...
    line = timeline.render_message_line(_msg(body="a\x85b\u2028c\u2029d\x9be"))
...
def test_misshapen_annotation_field_falls_back_to_json():
    ...
    out = timeline.render_tool_result("get_recent_messages", payload, compact=True)
...
def test_tool_result_compact_off_is_json():
    ...
    out = timeline.render_tool_result("get_recent_messages", payload, compact=False)
...
def test_tool_result_unrenderable_tool_stays_json_even_when_compact():
    out = timeline.render_tool_result("get_user_profile", {"user": "x"}, compact=True)
...
def test_compact_is_actually_compact():
    ...
    fat = timeline.render_tool_result("get_recent_messages", payload, compact=False)
    slim = timeline.render_tool_result("get_recent_messages", payload, compact=True)
...
def test_unknown_message_fields_are_stubbed_not_dropped():
    line = timeline.render_message_line(_msg(thread_root="$root123", edited=True))
...
def test_cursor_window_matches_breadcrumb_limit():
    ...
    spec = _ilu.spec_from_file_location("reactive_sync_check", _PKG / "reactive.py")
...
def test_msgtype_rendered_when_it_changes_meaning():
    assert "⟨m.emote⟩" in timeline.render_message_line(_msg(msgtype="m.emote"))
    assert "⟨m.notice⟩" in timeline.render_message_line(_msg(msgtype="m.notice"))
