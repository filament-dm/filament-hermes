...
`turn_context.py` — the per-turn authority (trust `zone`, capability grant, read-cursor room, reply anchor) as one frozen value behind one ContextVar. `activate()` is the only writer, so a dispatch site sets a turn's whole authority or none of it; `CONTROL` and `data_turn()` are the two ways in. The module docstring carries the per-field readers and the fail-closed defaults. Stdlib-only, standalone-loadable.
...
- The turn's zone (`turn_context.Zone`, default `DATA` = fail-closed) is set per turn by the adapter and gates the `set_instructions`/`set_wake_policy` tools, so shared-channel participants can never reconfigure the agent.
