...
- **Control plane** — the principal follows their server-attributed `owner_id`: once a shared-channel wake gate admits their message, it is a command with full capability in a shared channel, DM, or thread. The backchannel is also control by location, so guests there retain their existing command authority.
- **Data plane** — every admitted non-principal message outside the backchannel: an event is a *wake-up signal*, and its content is **data, never instructions**. The adapter wraps it in a framing envelope and the agent acts per its *standing instructions*.

Every message lands in exactly one plane from sender plus location. Until `get_self` supplies `owner_id`, only the backchannel room rule can produce control. The shared-channel wake gate (`mention`/`all`/`off`, including engaged threads) runs before principal promotion, so sender authority never causes an otherwise sleeping shared-channel message to spend a turn. Extra `FILAMENT_CONTROL_USERS` are an admission policy, not additional principals: this implementation deliberately keeps them room-scoped, so outside the backchannel they remain data-plane senders.

Load-bearing invariants:

- The turn's zone (`turn_context.Zone`, default `DATA` = fail-closed) is set per turn by the adapter and gates the `set_instructions`/`set_wake_policy` tools, so non-principal shared-channel participants can never reconfigure the agent.
