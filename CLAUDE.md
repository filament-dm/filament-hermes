...
`FILAMENT_MCP_TOKEN` (required), `FILAMENT_MCP_URL` (default production `https://api.filament.dm/mcp/agents`), `FILAMENT_CONTROL_USERS` (extra trusted commanders; the principal is auto-discovered via `get_self`), `FILAMENT_ALLOW_DATA_USERS` (default true — set false for a control-plane-only agent), `FILAMENT_HOME_ROOM`, `FILAMENT_FCM_CREDENTIALS_DIR`, `FILAMENT_CAPABILITY_POLICY_FILE` (override the data-plane capability policy path; default `<creds dir>/capability_policy.json`), `FILAMENT_FEATURE_FLAGS_FILE` (override the feature-flag path; default `<creds dir>/feature_flags.json`), `FILAMENT_ENGAGED_THREADS_FILE` (override the engaged-thread record path; default `<creds dir>/engaged_threads.json`), `FILAMENT_DISABLE_UPDATE_CHECK` (set true to turn off the daily new-version check — see `update_check.py`), `FILAMENT_DISABLE_AUTO_UPDATE` (set true to keep the check but not act on it: the agent reminds the principal to update by hand instead of updating itself — see `self_update.py`), `FILAMENT_UPDATE_CHECK_URL` (override the pyproject.toml URL the update check fetches — a test seam so a harness can stand in for "the version on main"; production never sets it), `HERMES_HOME`.

## Versioning

Every HTTP request to the Filament server carries the installed plugin version (`User-Agent` + `X-Filament-Plugin-Version` headers set client-wide in `filament_api.py`, plus MCP `clientInfo` on `initialize`) so the server can tell what version deployed agents run. `_version.py` owns the helpers (stdlib-only). Installs come from git main, so **bump `version` in pyproject.toml on every user-visible change** — `update_check.py` compares the installed version against pyproject.toml on main daily, and a newer version triggers `self_update.py`.

`self_update.py` — the daily check acts on its own by default: `git pull --ff-only` the plugin tree, then restart the gateway into it (`GatewayRunner.request_restart`, the same path Hermes' own `/restart` uses, so in-flight agent turns drain first). The principal is told, not asked — one backchannel message before, one after. The "after" message cannot be sent by the process that upgrades (it dies mid-way), so the target version and room go into `pending_upgrade.json` *before* the restart and the process that comes back announces the result on connect, reporting the version actually running. Loop guards, both load-bearing: `mark_notified(latest)` fires before anything fallible, so a broken upgrade can't become a daily restart; the marker is cleared on the connect that reads it, so an upgrade can't re-announce forever. A pip install (no `.git`) or `FILAMENT_DISABLE_AUTO_UPDATE` falls back to the once-per-version reminder (`update_check.build_reminder`). `adapter.py` can't be imported without Hermes, so its half of this is pinned by source/AST assertions in `tests/test_self_update.py`.
